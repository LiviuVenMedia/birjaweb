...
const theme = createTheme({
  ...
  components: {
    MuiPaper: {
      styleOverrides: {
        root: ({ theme }) => ({
          background: `linear-gradient(180deg, ${alpha(theme.palette.background.paper, 0.85)} 0%, ${alpha(
  ...
  useEffect(() => {
    if (vacancies.length === 0) {
      setVacancies([
        {
          id: 1,
          ownerId: 'demoEmployer',
          title: 'Șofer categoria B',
          region: 'Chișinău',
          salary: '10000 MDL',
          text: 'Livrări locale, program flexibil.',
        },
        {
          id: 2,
          ownerId: 'demoEmployer',
          title: 'Operator call-center',
          region: 'Bălți',
          salary: '8000 MDL',
          text: 'Program 9-18, limba română și rusă.',
        },
        {
          id: 3,
          ownerId: 'otherCo',
          title: 'Programator Frontend',
          region: 'Remote / Moldova',
          salary: '1200 EUR',
          text: 'React, TypeScript, full remote.',
        },
      ])
    }
  }, [setVacancies])
            ...
            <Route
              ...
              element={
                <Paper
                  ...
                  className='p-6'
                  aria-live='polite'
                  sx={{ maxWidth: { xs: '100%', sm: 720, md: 900 }, width: '100%', mb: 4, mx: 'auto', p: 2 }}
                >
                  <Typography variant='h5' marginBottom={3} marginTop={2}>
                    All vacantions
                  </Typography>
                  <VacancyList
                    items={vacancies}
                  ...
                  {!currentUser && (
                    <>
                      <Typography variant='h6' sx={{ marginTop: 4, marginBottom: 2, pl: 1 }}>
            ...
            <Route
              ...
              element={
                <Paper
                  ...
                  className='p-6'
                  aria-live='polite'
                  sx={{ maxWidth: { xs: '100%', sm: 720, md: 900 }, width: '100%', mx: 'auto', p: 2 }}
                >
                ...
                currentUser ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      <Typography variant='caption' sx={{ display: 'inline-block', mt: 2 }}>
...
function ImageUpload({ images, setImages }: { images: string[]; setImages: React.Dispatch<React.SetStateAction<string[]>> }) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      const token = (() => {
        try {
          return JSON.parse(localStorage.getItem('birjatg_currentUser') || '{}')?.token || ''
        } catch {
          return ''
        }
      })()
...
function CandidateList({ items, onEdit }: CandidateListProps) {
  ...
  return (
    <Box id='employer-candidates' sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant='h6'>Candidates</Typography>
        <Button size='small' variant='contained' color='primary' onClick={() => setShowAddForm(true)}>
          Add new candidate
        </Button>
      </Box>
                ...
                const resp = await fetch('/api/employer/candidates', {
                  ...
                  headers: {
                    ...
                    Authorization: `Bearer ${
                      localStorage.getItem('birjatg_currentUser')
                        ? JSON.parse(localStorage.getItem('birjatg_currentUser')!).token
                        : ''
                    }`,
                  },
            ...
            {editingId === a.id ? (
              ...
              >
                ...
                <TextField
                  label='Interes'
                  fullWidth
                  defaultValue={a.interest || ''}
                  onChange={e => setForm(p => ({ ...p, interest: e.target.value }))}
                />
                <TextField
                  label='Contract'
                  fullWidth
                  defaultValue={a.Contract || ''}
                ...
                <TextField
                  label='SalaryWorker'
                  fullWidth
                  defaultValue={a.SalaryWorker || ''}
                  onChange={e => setForm(p => ({ ...p, salaryWorker: e.target.value }))}
                />
                <FormControl fullWidth>
                  <InputLabel id={`status-label-${a.id}`}>Status</InputLabel>
                  <Select
                    labelId={`status-label-${a.id}`}
                    label='Status'
                    defaultValue={a.status || 'new'}
                    onChange={e => setForm(p => ({ ...p, status: e.target.value as string }))}
                  >
